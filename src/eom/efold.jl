...
function friedmann_eq_efold(u, p, t)
    ...
    return SA[dϕdN, -3 * dϕdN - get_dV(ϕ) / get_H2(ϕ, dϕdN, get_V), a]
end
...
function solve_ode(u₀::SVector{3, Float64}, 
    ...
    condition(u, t, integrator) = u[2]^2 / (2) <= 0.5
    affect!(integrator) = terminate!(integrator)
    cb = ContinuousCallback(condition,affect!)

    prob = ODEProblem(friedmann_eq_efold, u₀, [0.0, 10.0], p)
    sol = solve(prob, RK4(), reltol=1e-6, abstol=1e-8, dtmax=0.001, callback=cb)
